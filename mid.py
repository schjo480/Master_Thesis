...
class MID():
    ...
    def train(self):
        for epoch in range(1, self.epochs + 1):
            # self.train_dataset.augment = self.augment
            ...
            if epoch % self.eval_every == 0:
                ...
                torch.save(checkpoint, osp.join(self.model_dir, f"{self.dataset}_epoch{epoch}.pt"))
        ...
        for i, scene in enumerate(self.eval_scenes):
            ...
            for t in tqdm(range(0, scene.timesteps, 10)):
                ...
                '''if t < 500:
                    ...
                    plt.savefig(f"./experiments/tdrive_edge_coordinates_experiment/eval_plots/plot_{i}_{t}.png")
                    plt.close(fig)'''
    ...
    def _build_dir(self):
        self.exp_name = 'mid_tdrive_edge_coordinates_experiment'
    ...
    def _build_encoder_config(self):
        ...
        self.hyperparams['enc_rnn_dim_edge'] = self.encoder_dim//2
        self.hyperparams['enc_rnn_dim_edge_influence'] = self.encoder_dim//2
        self.hyperparams['enc_rnn_dim_history'] = self.encoder_dim//2
        self.hyperparams['enc_rnn_dim_future'] = self.encoder_dim//2
        # registar
        self.registrar = ModelRegistrar(self.model_dir, "cuda")

        if self.eval_mode:
            epoch = self.eval_at
            checkpoint_dir = osp.join(self.model_dir, f"{self.dataset}_epoch{epoch}.pt")
            self.checkpoint = torch.load(osp.join(self.model_dir, f"{self.dataset}_epoch{epoch}.pt"), map_location = "cpu")

            self.registrar.load_models(self.checkpoint['encoder'])


        with open(self.train_data_path, 'rb') as f:
            self.train_env = dill.load(f, encoding='utf-8')
        with open(self.eval_data_path, 'rb') as f:
            self.eval_env = dill.load(f, encoding='utf-8')
    ...
    def _build_train_loader(self):
        ...
        for attention_radius_override in self.override_attention_radius:
        ...
        self.train_scenes_sample_probs = self.train_env.scenes_freq_mult_prop if self.scene_freq_mult_train else None
        ...
        self.train_dataset = EnvironmentDataset(train_env,
                                           ...
                                           return_robot=not self.incl_robot_node)
    ...
    def _build_eval_loader(self):
        ...
        if self.eval_every is not None:
            ...
            eval_scenes_sample_probs = self.eval_env.scenes_freq_mult_prop if self.scene_freq_mult_eval else None
