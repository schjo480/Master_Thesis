...
class TrajectoryDataset(Dataset):
    ...
    def generate_edge_features(self, history_indices, future_indices, history_edge_orientations=None, future_edge_orientations=None):
        ...
        if 'pos_encoding' in self.edge_features:
            encoding_tensor = torch.zeros((len(self.edges), self.embedding_dim), dtype=torch.float64)
...
class Edge_Encoder_MLP(nn.Module):
    ...
    def forward(self, x, t=None, condition=None, mode=None, history_indices=None):
        """
...
class Graph_Diffusion_Model(nn.Module):
    ...
    def _build_train_dataloader(self):
        ...
        self.train_dataset = TrajectoryDataset(self.train_data_path, self.history_len, self.future_len, self.edge_features, device=self.device, embedding_dim=4)
    ...
    def _build_test_dataloader(self):
        self.val_dataset = TrajectoryDataset(self.test_data_path, self.history_len, self.future_len, self.edge_features, device=self.device, embedding_dim=4)
...
data_config = {"dataset": "synthetic_20_traj",
    "train_data_path": '/ceph/hdd/students/schmitj/MA_Diffusion_based_trajectory_prediction/data/tdrive_val.h5',
